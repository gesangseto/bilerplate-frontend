...
const ReportAuditTrail = () => import("@/views/reports/ReportAuditTrail");
...
function menuToRouter(item) {
  const makeJSON = (data) => {
    let _tmp = JSON.parse(data);
    _tmp = _tmp[0]._children;
    return _tmp;
  };
  const reformatRoute = (data) => {
    let parent = [];
    for (const it of data) {
      ...
      if (it.link) {
        _parent = {
          path: it.link,
          name: `${it.name} (Details)`,
          component: eval(it.func_name ?? null),
          meta: { login: true },
        };
      }
      parent.push(_parent);
  ...
  let new_item = makeJSON(item);
  new_item = reformatRoute(new_item);
  return;
}
