...
export default {
  ...
  methods: {
    // hangeAnyChange(pids) {
    //   for (var i = 0; i < this.packaging_level == 1 ? 1 : 2; i++) {
    //     if (pids[i].epc_type === 'sscc') {
    //       this.checkSSCC(i);
    //     } else if (pids[i].epc_type === 'sgtin') {
    //       this.checkSGTIN(i);
    //     } else if (pids[i].epc_type === 'nie') {
    //     }
    //   }
    // },
    syncPidID() {
      for (var i = 0; i < this.result.length; i++) {
        this.error[i] = this.initial_error();
        this.result[i].error = false;
        let datanya = this.result[i];
        if (datanya.epc_type === 'nie') {
          this.checkNIE(i);
        } else if (datanya.epc_type === 'sgtin') {
          this.checkSGTIN(i);
        } else if (datanya.epc_type === 'sscc') {
          this.checkSSCC(i);
        }
      }
    },
    ...
    checkNIE(num) {
      this.result[num].id1 = this.item.nie;
      this.result[num].id2 = '';
      this.result[num].id3 = '';
      if (!this.result[num].id1) {
        this.result[num].error = true;
        this.error[num].id1 = false;
      } else {
        this.result[num].error = false;
        this.error[num].id1 = false;
      }
    ...
    checkSSCC(num) {
      let data = this.item;
      if (!isNumeric(this.result[num].id1)) {
        this.result[num].id1 = '';
      }
      this.result[num].id2 = data.company_prefix;
      this.result[num].id3 = '';
    ...
    checkSGTIN(num) {
      let data = this.item;
      this.result[num].error = null;
      if (data.item_reference) {
        if (this.packaging_level == 1) {
          this.result[num].id1 = data.item_reference.charAt(0);
        } else {
          this.result[num].id1 = this.result[num].id1 || '';
        }
        this.result[num].id2 = data.company_prefix;
        this.result[num].id3 = data.item_reference.substring(1);
      } else {
        this.result[num].id1 = null;
        this.result[num].id2 = data.company_prefix;
      ...
      console.log(this.result[num], this.error[num]);
      return this.result[num];
    },
