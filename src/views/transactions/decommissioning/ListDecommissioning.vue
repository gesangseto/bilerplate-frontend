<template>
  <CRow>
    <CCol col="12" xl="12">
      <CCard>
        <CCardHeader>
          <h5>Commissioning / Decommissioning</h5>
        </CCardHeader>
        <CCardBody>
          <CRow>
...
export default {
  ...
  methods: {
    ...
    rowViewClicked(item) {
      this.$router.push({ path: `comm-decomm/read/${item.id}` });
    },
    rowUpdateClicked(item) {
      this.$router.push({ path: `comm-decomm/approve/${item.id}` });
    },
  },
  computed: {
    decomissioning() {
