...
const AIs = [
  //FIX_AI = [
  { ai: "00", type: "SSCC", length: 18, alias: "sscc" },
  { ai: "01", type: "GTIN", length: 14, alias: "gtin" },
...
export const parsingDataToBarcode = ({ data_object = Object }) => {
  ...
  for (const it in data_object) {
    for (const ai of AIs) {
      if (ai.alias === it && data_object[it]) {
        let data = data_object[it];
        let date = moment(data);
        if (date.isValid()) {
          data = moment(date).format("YYMMDD");
        }
        str += `(${ai.ai})${data}`;
      }
    }
